...
class DataAnalyzer:
    ...
    def analyze_product_distribution(
        ...
        # Create visualization (use non-interactive backend at runtime)
        import matplotlib
    ...
    def analyze_narrative_lengths(
        ...
        length_stats = {
            ...
            "total_empty": df[narrative_col].isna().sum()
            + (df[narrative_col] == "").sum(),
        }
        ...
        axes[0].axvline(
            ...
            color="red",
            linestyle="--",
            label=f"Mean: {length_stats['mean']:.0f}",
        )
        axes[0].axvline(
            length_stats["median"],
            color="green",
            linestyle="--",
            label=f"Median: {length_stats['median']:.0f}",
        )
    ...
    def generate_summary_report(
        ...
        report["product_distribution"] = self.analyze_product_distribution(
            df, product_col
        )
